...
func ToStatus(err error) int {
	...
	case errors.Is(err, ErrNotFound):
		return 404
	case errors.Is(err, ErrDuplicate), errors.Is(err, ErrRepeatedRegion), errors.Is(err, ErrSnapshotFrozen):
		return 409
	case errors.Is(err, ErrStateTransition), errors.Is(err, ErrArchivedMutation), errors.Is(err, ErrPeakSealed), errors.Is(err, ErrAnalyzeLocked):
		return 409
	case errors.Is(err, ErrPolarizationMissing), errors.Is(err, ErrCoordinateRange), errors.Is(err, ErrBadRequest):
		return 400
	default:
		return 500
	}
